    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      >
        <div className="grid lg:grid-cols-12 gap-12 items-center min-h-[80vh]">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }} className="lg:col-span-7 space-y-8">
            ...
            <motion.h1 className="text-5xl sm:text-6xl lg:text-7xl xl:text-8xl font-bold leading-[1.1] tracking-tight" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <motion.p className="text-xl sm:text-2xl text-text/70 leading-relaxed max-w-2xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.8 }}>
              {HOME.hero.description}
            </motion.p>
            <motion.div className="flex flex-col sm:flex-row gap-4 pt-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.8 }}>
              <Link href={HOME.hero.cta.primary.href} className="group relative">
                <motion.div className="relative px-8 py-4 bg-gradient-to-r from-accent to-accent/90 text-white rounded-2xl font-semibold text-lg shadow-glow overflow-hidden" whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }}>
              ...
              <Link href={HOME.hero.cta.secondary.href} className="group relative">
                <motion.div className="relative px-8 py-4 glass border-2 border-dark/10 text-dark rounded-2xl font-semibold text-lg shadow-soft overflow-hidden group-hover:border-accent/30 transition-all" whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }}>
                  <span className="relative z-10">{HOME.hero.cta.secondary.text}</span>
                  <motion.div className="absolute inset-0 bg-gradient-to-r from-accent/0 via-accent/10 to-accent/0" initial={{ scale: 0 }} whileHover={{ scale: 1 }} transition={{ duration: 0.4 }} />
                </motion.div>
              </Link>
            </motion.div>
          </motion.div>
              ...
              {HOME.stats.map((stat, index) => (
                ...
                >
                  <div className="glass rounded-3xl p-6 border border-white/50 shadow-soft hover:shadow-hover transition-all duration-300 h-full">
                    ...
                    <motion.div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-accent/0 to-primary/0 group-hover:from-accent/10 group-hover:to-primary/10 transition-all duration-300 -z-10" />
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <motion.div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-10" animate={{ y: [0, 10, 0] }} transition={{ duration: 2, repeat: Infinity }}>
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="w-6 h-10 border-2 border-text/20 rounded-full flex justify-center p-1">
            <motion.div className="w-1.5 h-1.5 bg-accent rounded-full" animate={{ y: [0, 20, 0] }} transition={{ duration: 1.5, repeat: Infinity }} />
          </div>
        </div>
      </motion.div>
    </section>
