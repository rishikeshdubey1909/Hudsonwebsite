...
export function initSectionReveals() {
  ...
  sections.forEach((section) => {
    ...
    gsap.set(section, { opacity: 0, y: 20, force3D: true })
    if (children.length > 0) {
      gsap.set(children, { opacity: 0, y: 15, force3D: true })
    }
    ...
    // Animate section with GPU acceleration
    tl.to(section, {
      opacity: 1,
      y: 0,
      force3D: true,
...
export function initCardReveals() {
  ...
  containers.forEach((container) => {
    ...
    // Animate cards with GPU acceleration
    gsap.to(cards, {
      opacity: 1,
      scale: 1,
      y: 0,
...
export function initAllGSAPAnimations() {
  ...
  // Refresh on resize with debounce (increased for better performance)
